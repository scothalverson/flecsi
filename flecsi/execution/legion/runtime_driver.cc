...
void
runtime_driver(const Legion::Task * task,
  ...
  if(number_of_global_fields > 0) {
    ...
    size_t global_index_space = execution::internal_index_space::global_is;
  ...
  for(size_t color(0); color < num_colors; ++color) {
    ...
    args_serializers[color].serialize(&num_sparse_index_spaces, sizeof(size_t));
    ...
    for(auto & itr : context_.adjacency_info()) {
      ...
      auto t =
        std::make_tuple(ai.index_space, ai.from_index_space, ai.to_index_space);
    ...
    args_serializers[color].serialize(
      &adjacencies_vec[0], num_adjacencies * sizeof(adjacency_triple_t));
  ...
  if(number_of_global_fields > 0) {
    ...
    ispace_dmap[global_index_space].entire_region =
      global_ispace.logical_region;
  } // if

  auto color_ispace = data.color_index_space();

  if(number_of_color_fields > 0) {
    size_t color_index_space = execution::internal_index_space::color_is;
...
void
setup_rank_context_task(const Legion::Task * task,
  const std::vector<Legion::PhysicalRegion> & regions,
  Legion::Context ctx,
  Legion::Runtime * runtime) {
  using namespace data;
  ...
  clog_assert(
    task->arglen > 0, "setup_rank_context_task called without arguments");
  ...
  args_deserializer.deserialize(
    field_info_buf, sizeof(field_info_t) * num_fields);
  ...
  args_deserializer.deserialize(
    (void *)adjacencies, sizeof(adjacency_triple_t) * num_adjacencies);
  ...
  */
  ...
  if(number_of_sparse_fields > 0) {
    ...
    for(const field_info_t & fi : context_.registered_fields()) {
      ...
      auto ac =
        pr.get_field_accessor(fi.fid).template typeify<sparse_field_data_t>();
